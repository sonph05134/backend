	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			while (iter.hasNext()) {
				...
				if (inputName.endsWith("id")) {
					id = item.getString(CommonConst.REQEUST_CHARACTER_ENCODING_UTF8);
				} else if (inputName.endsWith("code")) {
		 ...
		 RequestDispatcher dispatcher =
		 request.getServletContext().getRequestDispatcher("/views/product/edit.jsp");
		 dispatcher.forward(request, response);
		 return;
		 }
